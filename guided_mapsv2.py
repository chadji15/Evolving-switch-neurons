...
from guided_maps import calculate_weights, reorder_inputs
...
class GuidedMapv2ConnectionGene(BaseGene):
...
class GuidedMapv2NodeGene(DefaultNodeGene):

    _gene_attributes = [BoolAttribute('is_isolated'), #Map vs isolated neuron
                        FloatAttribute('bias'),
                        StringAttribute('type')]

    def distance(self, other, config):
        d = int(self.is_isolated == other.is_isolated) + int(self.type == other.type)
        return d * config.compatibility_weight_coefficient

class GuidedMapv2Genome(DefaultGenome):
    @classmethod
    def parse_config(cls, param_dict):
        param_dict['node_gene_type'] = GuidedMapv2NodeGene
        param_dict['connection_gene_type'] =GuidedMapv2ConnectionGene
...
class GatingNeuron(Neuron):

    def __init__(self, key, std_weights, bias):
        self.key = key
        self.params = {
        ...
        super().__init__(self.key, self.params)
...
def create(genome, config, map_size, inner_maps):
    ...
    for cg in itervalues(genome.connections):
        ...
        if cg.is_mod and genome.nodes[o].type == 'switch':
    ...
    for node_key in sorted_keys:
        ...
        if node_key in genome.nodes:
            ...
            for n in node_map:
                ...
                if node.type == 'gating':
                    nodes.append(GatingNeuron(n, std_weights=std_inputs[n], bias= node.bias))
                elif node.type == 'linear':
                    nodes.append(LinearNeuron(n, std_weights=std_inputs[n], bias=node.bias))
            ...
            for n in node_map:
                nodes.append(LinearNeuron(n, std_weights=std_inputs[n], bias=0))
...
def run(config_file, generations, binary_file, drawfile, progressfile, statsfile):
    ...
    eval_func = partial(eval_one_to_one_3x3, num_episodes=200, rand_iter=40, snapshot_inter=3, descriptor_out=False,
    ...
    # Load configuration.
    config = neat.Config(GuidedMapv2Genome, neat.DefaultReproduction,
